...
class SharedPreferencesFaceRecognitionPreferencesTest {

    private lateinit var preferencesStore: InMemorySharedPreferences
    private lateinit var preferences: SharedPreferencesFaceRecognitionPreferences

    @Before
    fun setUp() {
        preferencesStore = InMemorySharedPreferences()
        preferences = SharedPreferencesFaceRecognitionPreferences(preferencesStore)
    }

    @Test
    fun defaultsToEnabled() {
        assertTrue(preferences.isFaceRecognitionEnabled())
    }

    @Test
    fun persistsUpdatedValue() {
        preferences.setFaceRecognitionEnabled(false)

        assertFalse(preferences.isFaceRecognitionEnabled())
        assertFalse(preferencesStore.getBoolean("face_recognition_enabled", true))
    }
}
